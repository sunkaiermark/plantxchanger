...
export function EquipmentCard({ equipment }: { equipment: EquipmentSummary }) {
  return (
    <article className="group overflow-hidden border border-[#e2e2e2] bg-white shadow-[0_2px_5px_rgba(0,0,0,0.10)] transition hover:border-[#ff3d00] hover:shadow-[0_4px_14px_rgba(0,0,0,0.16)]">
      ...
      <div className="p-7">
        <h3 className="min-h-[4.5rem] text-[clamp(1.35rem,2vw,1.75rem)] font-black uppercase leading-tight tracking-normal text-[#202329]">
          <Link href={`/equipment/${equipment.slug}`}>{equipment.title}</Link>
        </h3>
        <div className="mt-6 grid grid-cols-2 gap-x-6 gap-y-4 font-mono">
          <div>
            <p className="text-[11px] font-black uppercase text-[#a8a8a8]">Make/model</p>
            <p className="mt-1 break-words text-lg text-[#4a4d52]">{compactModel(equipment)}</p>
          </div>
          <div>
            <p className="text-[11px] font-black uppercase text-[#a8a8a8]">Year</p>
            <p className="mt-1 text-lg text-[#4a4d52]">{equipment.year ?? "Review"}</p>
          </div>
          <div>
            <p className="text-[11px] font-black uppercase text-[#a8a8a8]">Location</p>
            <p className="mt-1 truncate text-lg text-[#4a4d52]">{equipment.country ?? "Global"}</p>
          </div>
          <div>
            <p className="text-[11px] font-black uppercase text-[#a8a8a8]">Status</p>
            <p className="mt-1 text-lg text-[#4a4d52]">
              {availabilityLabels[equipment.availability] ?? equipment.availability}
            </p>
          </div>
        </div>
      </div>
      <div className="flex min-h-16 items-center justify-between gap-4 border-t border-[#eeeeee] bg-[#fbfbfb] px-7 py-4">
        <p className="font-mono text-[clamp(1.25rem,1.6vw,1.65rem)] font-black uppercase text-[#202329]">
          {formatPrice(equipment)}
        </p>
        <Link
          href={`/equipment/${equipment.slug}`}
          className="hidden shrink-0 font-mono text-sm font-black uppercase tracking-[0.12em] text-[#ff7a55] group-hover:inline-flex"
        >
          View details -&gt;
        </Link>
      </div>
